static const s16 koopa_npc_anim_Boss_Jump_values[] = {
	0x0000, 0xFFFF, 0x0000, 0xFFFF, 0xFFFF, 0xFFFF, 0x0000, 0x0000, 0x0000, 
	0x1650, 0x0000, 0xFFFF, 0xFA0E, 0xFDA0, 0x042E, 0x1F95, 0xF7C4, 0xF5A8, 
	0xEC3E, 0xFD36, 0xF4CE, 0x0419, 0x0114, 0xF482, 0xFB15, 0x0EFE, 0x2BBC, 
	0x1689, 0xFC60, 0x0CA9, 
};

static const u16 koopa_npc_anim_Boss_Jump_indices[] = {
	0x0001, 0x0000, 0x0001, 0x0001, 0x0001, 0x0002, 0x0001, 0x0003, 0x0001, 
	0x0004, 0x0001, 0x0005, 0x0001, 0x0006, 0x0001, 0x0007, 0x0001, 0x0008, 
	0x0001, 0x0009, 0x0001, 0x000A, 0x0001, 0x000B, 0x0001, 0x000C, 0x0001, 
	0x000D, 0x0001, 0x000E, 0x0001, 0x000F, 0x0001, 0x0010, 0x0001, 0x0011, 
	0x0001, 0x0012, 0x0001, 0x0013, 0x0001, 0x0014, 0x0001, 0x0015, 0x0001, 
	0x0016, 0x0001, 0x0017, 0x0001, 0x0018, 0x0001, 0x0019, 0x0001, 0x001A, 
	0x0001, 0x001B, 0x0001, 0x001C, 0x0001, 0x001D, 
};

static const struct Animation koopa_npc_anim_Boss_Jump = {
	0,
	0,
	0,
	1,
	1,
	ANIMINDEX_NUMPARTS(koopa_npc_anim_Boss_Jump_indices),
	koopa_npc_anim_Boss_Jump_values,
	koopa_npc_anim_Boss_Jump_indices,
	0,
};

