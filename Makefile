...
DEBUG_MAP_STACKTRACE_FLAG := -DDEBUG_MAP_STACKTRACE
...
ifeq ($(COMPILER),gcc)
  CC      := $(CROSS)gcc
  $(BUILD_DIR)/actors/%.o:           OPT_FLAGS := -Ofast -mlong-calls -Wno-overflow
  $(BUILD_DIR)/levels/%.o:           OPT_FLAGS := -Ofast -mlong-calls -Wno-overflow
else ifeq ($(COMPILER),clang)
  CC      := clang
endif
...
$(BUILD_DIR)/include/text_strings.h: include/text_strings.h.in $(BUILD_DIR)/include/text_menu_strings.h
	$(call print,Encoding:,$<,$@)
	$(V)$(CPP) -P -I $(BUILD_DIR)/include -o $@ $<
$(BUILD_DIR)/include/text_menu_strings.h: include/text_menu_strings.h.in
	$(call print,Encoding:,$<,$@)
	$(V)$(CPP) -P -I $(BUILD_DIR)/include -o $@ $<
$(BUILD_DIR)/text/%/define_courses.inc.c: text/define_courses.inc.c text/%/courses.h
	@$(PRINT) "$(GREEN)Preprocessing: $(BLUE)$@ $(NO_COL)\n"
	$(V)$(CPP) $(CPPFLAGS) $< -o $@ -I text/$*/
